...
class FacetedSearch {
    ...
    onSortBySubmit(event) {
        ...
        // Formerly delete url.query.page;
        delete url.query.search_query;

        event.preventDefault();

        urlUtils.goToUrl(Url.format({
    ...
    applyswatches() {
        Papa.parse("/content/swatches/color_swatches.csv", {
            download: true,
            header: true,
            complete: (results) => {
                $('#facetedSearch-content--Color .color-swatch-title').each((i, el) => {
                    const swatches = results.data;
                    for (let i = 0; i < swatches.length; i++) {
                        if (el.innerText.toLowerCase() === swatches[i].label.toLowerCase()) {
                            $(el).prepend(`<div class="swatch" style="background-color: ${swatches[i].value}; border: 1px solid #c1c1c1;"></div>`);
                        }
                    }
                });
            }
