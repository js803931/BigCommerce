...
export default class Cart extends PageManager {
    ...
    bindCartEvents() {
        ...
        $('[data-cart-update-text]', this.$cartContent).on('blur', event => {
            const $target = $(event.currentTarget);
    ...
    bindGiftWrappingForm() {
        $('.giftWrapping-select').on('click', event => {
    ...
    totalSavings() {
        const pricesArr = $('.cart__rrp-price').text().split('$');
        const multiArr = $('.form-input--incrementTotal');
        const subtotal = $('.cart__subtotal').text().substring(1);
        ...
        for (var i = 0; i < pricesArr.length; i++) {
            multiplierArr.push(parseFloat(multiArr[i].value));
            newPricesArr.push(parseFloat(pricesArr[i]));
            totalDiscountPrice += newPricesArr[i] * multiplierArr[i];        
        }
        
        const totalSavings = parseFloat(totalDiscountPrice - newSubtotal).toFixed(2);

        $('.cart__total-savings').text(`$${totalSavings}`);
    }
}
